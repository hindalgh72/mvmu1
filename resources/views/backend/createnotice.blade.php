            ...
            <div class="row">
                ...
                <div class="col-sm-10">
                    <div class="box box-primary" style="margin-top:20px !important;">
                        ...
                        @endif
                        <form action="{{route('notice.noticesub')}}" method="post">
                            @csrf
                            <div class="box-body">
                                <div class="form-group @error('notice_name') has-error @enderror">
                                    <label for="">Notice Name</label>
                                    <input type="text" class="form-control" name="notice_name" id="notice_name" value="{{old('notice_name')}}">
                                    @error('notice_name')
                                ...
                                <div class="form-group @error('notice_type_id') has-error @enderror">
                                    ...
                                    <select name="notice_type_id" id="notice_type_id" class="form-control" value="old('notice_type_id')">
                                        <option value="">Select Role</option>
                                        @foreach ($notices as $notice)
                                            <option value="{{$notice->id}}">{{$notice->notice_type_name}}   
                                                {{-- {{$notice->id}} --}}
                                        @endforeach
                                    </select>
